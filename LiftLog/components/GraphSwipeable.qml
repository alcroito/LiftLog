...
Item {
    ...
    // Global selected period across all graphs.
    property int selectedPeriod: 0

    signal currentGraphChanged(string graphLabel)

    StatsGraphSliderModel {
        id: graphListModel
    }

    ListView {
        id: littleCircleThingiesList
        anchors.bottom: root.bottom
        anchors.bottomMargin: -25 * units.scale
        anchors.horizontalCenter: root.horizontalCenter

        height: contentItem.childrenRect.height
        width: contentItem.childrenRect.width

        spacing: 9 * units.scale
        clip: true
        ...
        onCurrentIndexChanged: {
            graphList.currentIndex = currentIndex
        }

        highlight: Rectangle {
            id: circleThingyBlack
            color: "black"
            width: 7 * units.scale
            height: 7 * units.scale
            radius: 7 * units.scale
            z: 2
        }
        ...
        delegate: Rectangle {
            ...
            MouseArea {
                anchors.fill: parent
                onClicked: {
                    circleThingy.ListView.view.currentIndex = index
                }
            }
        }
    }
    ...
    MouseArea {
        id: leftThingyArea
        anchors.right: littleCircleThingiesList.left
        anchors.verticalCenter: littleCircleThingiesList.verticalCenter
        width: 50 * units.scale
        height: littleCircleThingiesList.height + 20 * units.scale
        onClicked: {
            littleCircleThingiesList.decrementCurrentIndex()
        }
    }

    MouseArea {
        id: rightThingyArea
        anchors.left: littleCircleThingiesList.right
        anchors.verticalCenter: littleCircleThingiesList.verticalCenter
        width: 50 * units.scale
        height: littleCircleThingiesList.height + 20 * units.scale
        onClicked: {
            littleCircleThingiesList.incrementCurrentIndex()
        }
    }
    ...
    ListView {
        ...
        // Cache at least 2 delegates on the sides.
        cacheBuffer: graphList.width * 2

        clip: true
        model: graphListModel

        Component.onCompleted: {
            // Set the currentIndex to the item that is currently visible in the list view.
            graphList.currentIndex = 0
        }

        onMovementEnded: {
            // Set the currentIndex to the item that is currently visible in the list view.
            graphList.currentIndex = graphList.indexAt(graphList.contentX, graphList.contentY);
        }

        onCurrentItemChanged: {
            currentGraphChanged(currentItem.exerciseNameToShow)
            littleCircleThingiesList.currentIndex = currentIndex
        ...
        delegate: Rectangle {
            ...
            DialogTextBox {
                id: errorDialogTextBox
                showDialog: !bounds.boundsSet
                dialogText: qsTr("Sorry, there was a problem while the graph was being prepared.")
            }

            StatsGraphData {
                id: statsData
                period: selectedPeriod
                dataToShow: model.dataToShow
                idExerciseToShow: model.idExerciseToShow
                listIndex: model.listIndex

                onPeriodChanged: {
                    statsData.getStatsFromDB()
        ...
        //            computeXAndYAxisRanges(bounds)
            ...
            Repeater {
                ...
                Label {
                    ...
                    color: "#6e6e6e"
                    visible: bounds.boundsSet
                    enabled: bounds.boundsSet
                }
            }
            ...
            Canvas {
                id: graphGrid
                anchors.bottomMargin: 30 * units.scale
                ...
                Component.onCompleted: {
                    // This is to make sure the lines are painted when switching to a new graph in the list view.
                    // For some reason this is needed on iOS, but not on OSX.
                    requestPaint();
                }
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        width: bubbleText.width + 20 * units.scale
                        height: 50 * units.scale
                        color: "#8c498e"
                        x: {
                            var delta = dragHandle.x + width / 2 - graphGrid.width;
                            if (delta < 0) {
                                delta = 0;
                            }

                            return (-width / 2) + dragHandle.width / 2 - delta;
                        }
                        ...
                        states: [
                            State {
                                ...
                                PropertyChanges {
                                    target: bubbleTriangle; opacity: 0.8
                                }
                            }
                        ]
